...
if (( ahead > 0 )); then
  if git push origin "$BRANCH" >> "$WORKLOG" 2>&1; then
    ...
    # --- Only deploy to Vercel when there are NEW commits ---
    # (Previous "deploy every cycle" approach hit Vercel's free-tier limit of
    # 100 deploys/day. Now we only deploy when something actually changed.)
